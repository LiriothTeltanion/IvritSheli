...
**Staleness on a branch is a warning, not a failure — since 2026-09-12.** On a `pull_request` the CI passes `--allow-stale-manifest`, so an out-of-date digest is reported as `[WARN]` instead of blocking. This exists because the manifest indexes `frontend/package.json`, `package-lock.json`, `backend/pyproject.toml` and the `Dockerfile`, which means **every Dependabot PR was born failing** and Dependabot cannot regenerate anything. Nothing real was given up: inside a worktree Git already holds the canonical bytes, so a stale digest means "not regenerated", never "corrupt". The check stays fatal on `push` to `main`, and in an extracted package without `.git` — where the manifest is the only source of integrity — it is fatal always. Only that one case softens; a listed file missing from the index, a malformed line or an unlisted required file still fail everywhere. Pinned by three tests in `backend/tests/test_package_checksums.py`.
