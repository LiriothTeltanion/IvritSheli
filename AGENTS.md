...
**The generator reads the Git index, not the working tree.** It uses `git ls-files --stage` and `git cat-file`, so `git add` your change *before* running it. Running it first and staging afterwards produces a manifest for the previous content, and `--check` then keeps failing for a reason that looks like a bug and is not one.

**Staleness is a warning everywhere except a release — since 2026-09-14.** On pull requests *and* on pushes to `main` the CI passes `--allow-stale-manifest`, so an out-of-date digest is reported as `[WARN]` instead of blocking. It is fatal on a pushed `v*` tag and on a manual `workflow_dispatch` run, which is the button to check release readiness before tagging.

Until 2026-09-14 it was also fatal on `push` to `main`. That turned every merged dependency update into a red `main` and a "CI workflow run failed" notification with nothing broken: on 2026-09-14 eight Dependabot merges produced six of them while every test and lint passed. The manifest is a release artifact, so it is enforced where a release happens — the same contract the public profile uses for its tags. **Before tagging a release, stage the tree, run `python scripts/generate_checksums.py` and merge that first.**

The original 2026-09-12 reasoning still holds for pull requests: This exists because the manifest indexes `frontend/package.json`, `package-lock.json`, `backend/pyproject.toml` and the `Dockerfile`, which means **every Dependabot PR was born failing** and Dependabot cannot regenerate anything. Nothing real was given up: inside a worktree Git already holds the canonical bytes, so a stale digest means "not regenerated", never "corrupt". In an extracted package without `.git` — where the manifest is the only source of integrity — it is fatal always. Only that one case softens; a listed file missing from the index, a malformed line or an unlisted required file still fail everywhere. Pinned by three tests in `backend/tests/test_package_checksums.py`.
